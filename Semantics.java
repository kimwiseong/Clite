...
public class Semantics {
    ...
    /** Block의 각 member들에 대한 state를 반환 */
    ...
    /** Conditional에 대해 test의 state를 검사 */
    State M (Conditional c, State state) {
        if (M(c.test, state).boolValue()) //true면 thenbranch 검사
            return M(c.thenbranch, state);
        else //false일 경우 elsebranch 검사
            return M(c.elsebranch, state);
        // student exercise
    }

    /** Loop에 대해 test의 state를 검사 */
    State M (Loop l, State state) {
        if (M (l.test, state).boolValue( )) //true일 경우 반복문의 body 부분 검사
            return M(l, M (l.body, state));
        else return state; //false일 경우에 대한 state 반환, 즉 반복문이 끝난 state
        // student exercise
    }
    ...
    Value applyBinary (Operator op, Value v1, Value v2) {
        ...
        /** bool type은 대소 비교가 불가능하기 때문에 AND, OR연산을 추가하였다. */
